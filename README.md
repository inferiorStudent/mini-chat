...
如果是其它的语料类型，你可以按照自己的方式处理，本项目以txt为语料数据的格式。
...
训练好分词器之后，会在`/out`目录下输出相关的分词器文件，后续只需要通过如下调用即可：
```python
...
在没有庞大的GPU计算资源的情况下，本项目在线训练。由于担心网络波动导致训练中断，因此将数据分成多个文件，每训练完一个文件分片保存一次权重，方便下次继续训练。
