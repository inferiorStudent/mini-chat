...
class Attention(nn.Module):
    ...
    def forward(self, x, attention_mask: torch.tensor=None, position_ids=None, past_key_value=None, rotary_emb=None):
        ...
        # 原来的写法有bug: past_key_value = (k, v), 分析一下为啥
...
class SLMModel(SLMPretrainedModel):
    ...
    def forward(self, input_ids, attn_mask=None, past_key_values=None):
        ...
        if past_key_values is not None:
            # past_key_values[0][0].shape = (b, num_heads, s, head_dim)
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_outputs = layer(
                hidden_states, attn_mask=attn_mask, position_ids=position_ids, past_key_value=past_kv, rotary_emb=self.rotary_emb
            )
...
# ForCausalLM
class SLMForCausalLM(SLMPretrainedModel):
    def __init__(self, config: SLMConfig):
        super().__init__(config)
    ...
    def forward(self, input_ids, attn_mask=None, labels=None, past_key_values=None):
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            outputs = self.forward(generated, attn_mask=attn_mask, past_key_values=past_key_values)
            logits = outputs.logits[:, -1, :]  # 取最新一位的logits
